...
while true; do
  ...
  "$PYBIN" -c "import loguru,pandas,dotenv,alpaca.data.historical,alpaca.trading.client" 2>/dev/null || { echo "[supervisor] MISSING DEPS for $PYBIN; retry in 60s" >> logs/flush_bot_supervisor.log; sleep 60; continue; }
  ARGS=""
