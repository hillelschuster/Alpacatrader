...
Also provides:
    ...
    * Soft-annotation mapping (SPEC §8)
...
def map_soft_warnings(
    ...
    *,
    ...
    # ── Data / quote quality warnings (may be redundant
    #    with hard filters when they're added in Phase 3) ──
    ...
    # ── Price-action warnings (need Phase 4 classifier) ──
    if parabolic:
...
def soft_warning_multiplier(
    ...
    *,
    ...
    _MULTIPLIERS: dict[str, float] = {
        ...
        "stale_quote": 0.5,
        "wide_spread_caution": 1.0,  # hard filter manages this in Phase 3
        "spread_caution": 1.0,
        "parabolic": 0.5,
        "below_vwap": 1.0,  # context only
        "below_ema": 1.0,  # trend warning
        "lunch_window": 0.5,
        "halt_history_today": 1.0,  # tiny/scalp only, managed by mode
        "low_data_confidence": 1.0,  # sizing already adjusts
